// scholarship-detail.js — Vertical layout with hero overlay, info rows, about, expandable sections
initPage('scholarships');
var schId = new URLSearchParams(location.search).get('id');
var sch = null, selectedProgram = null, existingApp = null;

async function loadSchDetail() {
    if (!schId) { document.getElementById('detailPage').innerHTML = '<div class="empty-state"><div class="empty-state__icon">⚠️</div><div class="empty-state__title">No scholarship selected</div></div>'; document.getElementById('applyBar').innerHTML = ''; return; }
    try {
        var res = await apiFetch('scholarships/' + schId);
        if (!res) throw new Error('Network error');
        var json = await res.json();
        sch = json.data || json;
        if (!sch || !sch._id) throw new Error('Not found');
        document.title = (sch.title || 'Scholarship') + ' — EduKar';
        ...
        try {
            ...
            for (var i = 0; i < apps.length; i++) {
                var a = apps[i];
                if ((a.type || '').toLowerCase() !== 'scholarship') continue;
                var entity = a.scholarship;
                if (entity && entity._id === schId) {
                    existingApp = a;
                    break;
                }
            }
...
function filterProgs(type) {
    return (sch.programs || []).filter(function (p) {
        var t = (p.type || '').toLowerCase();
        if (type === 'bachelor') return t === 'bachelor' || t === 'bs';
        if (type === 'master') return t === 'master' || t === 'ms';
        if (type === 'phd') return t === 'phd';
        return false;
    });
}

function renderDetail() {
    var deadline = sch.deadline ? new Date(sch.deadline) : null;
    var isClosed = !(sch.isActive !== false) || (deadline && deadline < new Date());
    var loc = sch.address || [sch.city, sch.country].filter(Boolean).join(', ');
    var testDate = sch.testDate || sch.test_date;
    var interviewDate = sch.interviewDate || sch.interview_date;
    var website = sch.website;
    var thumbSrc = resolveAsset(sch.image || sch.thumbnail || sch.logo);
    ...
    if (thumbSrc) {
        html += '<img src="' + thumbSrc + '" alt="' + (sch.title || '') + '" style="width:100%; height:100%; object-fit:cover; image-rendering:high-quality;" onerror="this.style.display=\'none\'; this.parentElement.style.background=\'linear-gradient(135deg, #F97316, #2E8B57)\'">';
    } else {
        html += '<div style="width:100%; height:100%; background:linear-gradient(135deg, #F97316, #2E8B57);"></div>';
    }
    ...
    html += '<a href="scholarships" style="width:38px; height:38px; background:rgba(255,255,255,.15); border-radius:50%; display:flex; align-items:center; justify-content:center; color:#fff; text-decoration:none; backdrop-filter:blur(8px);"><i class="fas fa-arrow-left"></i></a>';
    html += '<span style="padding:4px 12px; border-radius:6px; font-size:10px; font-weight:900; text-transform:uppercase; letter-spacing:1.5px; color:#fff; background:var(--orange);">Scholarship</span>';
    html += '</div>';
    html += '<h1 style="font-size:clamp(20px, 2.5vw, 32px); font-weight:900; color:#fff; line-height:1.2; text-shadow:0 2px 8px rgba(0,0,0,.5);">' + escapeHtml(sch.title || 'Scholarship') + '</h1>';
    html += '</div></div>';
    ...
    html += '<div style="flex:1; min-width:180px; border:1px solid #FECACA; border-radius:12px; padding:12px 16px; display:flex; align-items:center; gap:10px;"><i class="fas fa-calendar-alt" style="color:#EF4444; font-size:16px; flex-shrink:0;"></i><div><div style="font-size:11px; color:#EF4444; font-weight:700; text-transform:uppercase;">Deadline</div><div style="font-size:14px; font-weight:800; color:#991B1B;">' + formatDate(sch.deadline) + '</div></div></div>';
    if (sch.applicationFee || sch.applicationFees) {
        var cur = sch.currency || '';
        html += '<div style="flex:1; min-width:180px; border:1px solid #E2E8F0; border-radius:12px; padding:12px 16px; display:flex; align-items:center; gap:10px;"><i class="fas fa-money-bill-wave" style="color:var(--primary); font-size:16px; flex-shrink:0;"></i><div><div style="font-size:11px; color:#94A3B8; font-weight:700; text-transform:uppercase;">Fee</div><div style="font-size:14px; font-weight:800; color:#0F172A;">' + (cur ? cur + ' ' : '') + (sch.applicationFee || sch.applicationFees) + '</div></div></div>';
    }
    if (sch.amount) html += '<div style="flex:1; min-width:180px; border:1px solid #E2E8F0; border-radius:12px; padding:12px 16px; display:flex; align-items:center; gap:10px;"><i class="fas fa-coins" style="color:#F97316; font-size:16px; flex-shrink:0;"></i><div><div style="font-size:11px; color:#F97316; font-weight:700; text-transform:uppercase;">Amount</div><div style="font-size:14px; font-weight:800; color:#0F172A;">' + (sch.currency || '') + ' ' + sch.amount + '</div></div></div>';
    ...
    html += '<p class="about-text" id="aboutText" style="-webkit-line-clamp:3;">' + escapeHtml(sch.description || 'No description available.') + '</p>';
    if ((sch.description || '').length > 80) {
    ...
    // ====== EXPANDABLE SECTIONS ======
    var sections = [
        { id: 'programs', icon: 'fa-graduation-cap', title: 'Available Programs (' + (sch.programs || []).length + ')', render: renderPrograms }
    ];
    if (sch.coverage && sch.coverage.length > 0) {
        sections.push({ id: 'coverage', icon: 'fa-wallet', title: 'Financial Coverage', render: renderCoverage });
    }
    var elig = sch.eligibility || {};
    if ((elig.minPercentage && String(elig.minPercentage).trim() !== '') || (elig.description && elig.description.trim() !== '')) {
        sections.push({ id: 'eligibility', icon: 'fa-check-circle', title: 'Eligibility Criteria', render: renderEligibility });
    }
    if (sch.provider && sch.provider.trim() !== '') {
        sections.push({ id: 'provider', icon: 'fa-building', title: 'Provider Info', render: renderProvider });
    }
    if (sch.contactInfo && sch.contactInfo.length > 0) {
...
function renderPrograms() {
    if (!(sch.programs || []).length) return '<p style="color:var(--slate-400); font-size:13px;">No programs available.</p>';
    return '<p style="font-size:12px; color:var(--slate-500); margin-bottom:12px;">Tap a program to select it before applying.</p>' +
...
function renderCoverage() {
    var coverage = sch.coverage || [];
    if (!coverage.length) return '<p style="color:var(--slate-400); font-size:13px;">No coverage details available.</p>';
    return coverage.map(function (c, i) {
        return '<div style="display:flex; gap:10px; margin-bottom:8px;"><span style="width:20px; height:20px; background:var(--primary-light); border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:10px; font-weight:900; color:var(--primary); flex-shrink:0;">' + (i + 1) + '</span><span style="color:var(--slate-600); font-size:13px; line-height:1.5;">' + escapeHtml(c) + '</span></div>';
    }).join('');
}

function renderEligibility() {
    var elig = sch.eligibility || {};
    var hasMin = elig.minPercentage != null && String(elig.minPercentage).trim() !== '';
    var hasDesc = elig.description && elig.description.trim();
    if (!hasMin && !hasDesc) return '<p style="color:var(--slate-400); font-size:13px;">No eligibility criteria available.</p>';
    return (hasMin ? '<div style="display:flex; justify-content:space-between; margin-bottom:10px;"><span style="font-size:13px;">Min Percentage</span><span style="font-weight:900; font-size:14px;">' + escapeHtml(String(elig.minPercentage)) + '%</span></div>' : '') +
        (hasDesc ? '<p style="color:var(--slate-600); line-height:1.7; white-space:pre-line; font-size:13px;">' + escapeHtml(elig.description) + '</p>' : '');
}

function renderProvider() {
    return '<div style="display:flex; justify-content:space-between;"><span style="font-size:13px;">Organization</span><span style="font-weight:900; font-size:14px;">' + escapeHtml(sch.provider || 'N/A') + '</span></div>';
}

function renderContacts() {
    var contacts = sch.contactInfo || [];
...
document.addEventListener('click', function (e) {
    ...
    if (item) {
        var p = filterProgs(item.dataset.type)[parseInt(item.dataset.idx)];
        if (p) {
            var n = p.name || p.programName;
            selectedProgram = { programName: n, programType: p.type, duration: p.duration };
            document.getElementById('progList').innerHTML = renderProgList(item.dataset.type);
...
async function applyNow() {
    ...
    try {
        ...
        var res = await apiFetch('applications/apply', { method: 'POST', body: { type: 'Scholarship', scholarshipId: schId, selectedPrograms: [selectedProgram] } });
        var data = res ? await res.json() : {};
        if (!res || !res.ok) throw new Error(data.message || 'Failed');
